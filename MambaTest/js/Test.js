function Testeur() {
    ...
    function possède(obj, prop) {
        return contient (Object.keys(obj), prop);
    }
    ...
    InstanceDeTest.DePosséder = function (ValeurAttendue) {
        var resultat = possède(this.ValeurATester, ValeurAttendue);
        logDeLErreur(resultat, this.ValeurATester, ValeurAttendue);
        this.ResultatDuTest &= resultat;
    }
    InstanceDeTest.DeNePasPosséder = function (ValeurAttendue) {
        var resultat = !possède(this.ValeurATester, ValeurAttendue);
        logDeLErreur(resultat, this.ValeurATester, ValeurAttendue);
        this.ResultatDuTest &= resultat;
    }
...
function Ca(label, test) {
    ...
    try {
        test();
        //Imprimer("Ca " + label + " : " + (InstanceDeTest.ResultatDuTest ? "VERT" : "ROUGE"));
		Imprimer("Ca " + label, InstanceDeTest.ResultatDuTest);
    }
    catch (err) {
   	    InstanceDeTest.ResultatDuTest = false;
   	    Imprimer("Ca " + label + " : Exception -> " + err.message, InstanceDeTest.ResultatDuTest);
...
function Imprimer(test, vert){
	console.log(test+ " : "+(vert ? "VERT":"ROUGE"));
}
...
Test(function () {
    ...
    Ca("possède une propriété", function () {
        OnAttend({'a' : 'vala', 'b' : 'valb'}).DePosséder('a');
        OnAttend({'a' : 'vala', 'b' : 'valb'}).DePosséder('b');
        OnAttend({'a' : 'vala', 'b' : 'valb'}).DeNePasPosséder('c');
    });
